...
function Details() {
  ...
  return isLoading || isLoadingHospital || isLoadingDoctors ? (
    ...
    <>
      <section>
        <Image
          src={getDepartment?.data?.data?.banner?.path || departmentFever}
          alt="Service Name"
        ...
        <div className="flex gap-5">
          <Image
            ...
            alt="Service Name"
            className=" object-cover h-[50px] md:h-[80px] w-[50px] md:w-[80px] rounded-[30px]"
            width={1000}
            height={1000}
          />
        ...
        <DescriptionParagraph
          content={getDepartment?.data?.data?.description || lorem100}
        />
        <Spacer y={3} />
          ...
          {getDoctors?.data?.data.map((d: any, index: any) => {
            return (
              <DoctorDetailCard
                ...
                data={d}
                redirect={
                  navigationRoutes.department + `${id}/doctor/` + "/" + d._id
                }
              />
            );
          })}
