...
function Tag({ name, number, onPress }: { name: string; number: number, onPress: () => any }) {
  return (
    <Chip onClick={onPress} className="rounded-none p-[.5rem] md:p-[1rem] bg-[#CDCDCD80] flex text-center hover:bg-primary text-black cursor-pointer hover:text-white">
...
function HospitalDetailscomponent({ id }: any) {
  ...
  return (
    (isLoading || isLoadingDepartment || isLoadingDoctors) ? <div className="flex flex-col h-[80vh] items-center justify-center">
      <Spinner />
        ...
        <div className="flex flex-col items-center justify-center bg-white rounded-xl shadow-xl w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 w-full gap-4 p-[1rem]">
            <div className="flex flex-col items-start">
              <h3 className="text-[14px] md:text-[24px] font-semibold">Address</h3>
              <p className="flex text-[10px] md:text-md flex-wrap">
                <span onClick={() => router.push(getHospitals?.data?.data?.locationUrl)} className="underline cursor-pointer font-semibold text-primary">
                  Get Directions
                </span>
              </p>
            </div>
            <div className="flex flex-col">
              <h3 className="text-[14px] md:text-[24px]  font-semibold">Timings</h3>
            ...
            <div className="flex text-[10px] md:text-md flex-col">
              ...
              <h3 className="flex flex-row gap-2">
                {getHospitals?.data?.data?.modesOfPayment.map((d: any, index: any) => {
                  return <h3 key={index}>{d} | </h3>
                })}
              </h3>
              <h3>Number of Beds - {getHospitals?.data?.data?.noOfBeds}</h3>
              <h3>Number of Ambulances - {getHospitals?.data?.data?.noOfAmbulances}</h3>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-4 bg-white shadow-xl rounded-xl w-full">
          <div className="flex flex-row w-full py-[1rem] justify-between items-center">
            ...
            <h3
              ...
              className="text-primary cursor-pointer text-[12px] md:text-[20px] underline decoration-solid "
            >
              View All
            </h3>
          </div>
